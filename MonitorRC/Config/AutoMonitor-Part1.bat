...
set SiteMonitor=suportelinx.lightning.force.com
set SiteNome=SalesForce
...
echo =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
echo.
echo REGISTRO DE ERRO: VERSÃO DO AUTOMONIRC RC:%AutoMoniVer%  SITE: %SiteNome% - URL: Monitorada %SiteMonitor% - DATA E HORA DO ERRO: %date% %time:~0,2%:%time:~3,2%:%time:~6,2% >> C:\MonitorRC\logs\%SiteNome%\%DATAHOJE%_%SiteNome%.txt
